...
### Practical 8: Performance Optimization & Code Splitting
- **Route-Based Lazy Loading**: Dynamic chunk loading with `React.lazy()` and `Suspense` for `/`, `/projects`, and `/contact`.
- **Meaningful Fallback UI**: Animated `PageLoader` spinner during route transitions with `lazyWithMinDelay` to prevent visual flicker.
- **Heavy Component Lazy Loading**: On-demand dynamic import of `ProjectAnalytics` (charts & statistics).
- **Vite Vendor Splitting**: Isolated `vendor-react` and `vendor-router` long-term caching chunks.
- **Evidence & Metrics**: Documented before/after build metrics and screenshots in [`PERFORMANCE_METRICS.md`](file:///c:/Users/Neel_Patel/Desktop/sem%205/AWDF/portfolio/PERFORMANCE_METRICS.md) and [`screenshots/`](file:///c:/Users/Neel_Patel/Desktop/sem%205/AWDF/portfolio/screenshots).
